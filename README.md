...
- **Iter21**. Добавление ассиметричного шифрования.
  - [x] Добавьте в агент и сервер поддержку асимметричного шифрования.
    - [x] Для агента: с помощью флага -crypto-key или переменной окружения CRYPTO_KEY передайте путь до файла с публичным ключом.
    - [x] Для сервера: с помощью флага -crypto-key или переменной окружения CRYPTO_KEY передайте путь до файла с приватным ключом.
    - [x] Шифруйте сообщения от агента к серверу с помощью ключей.
- **Iter22**. Конфигурация в формате JSON.
  - [x] Добавьте возможность конфигурации сервера и агента с помощью файла в формате JSON.
        **Нужно поддержать все действующие опции приложения.**
  - [x] Имя файла конфигурации должно задаваться через флаг -c/-config или переменную окружения CONFIG.
  - [x] Значения из файла конфигурации должны иметь меньший приоритет, чем флаги или переменные окружения.
- **Iter23**. Обработка сигналов.
  - [x] Агент должен штатно завершаться по сигналам: syscall.SIGTERM, syscall.SIGINT, syscall.SIGQUIT.
  - [x] Сервер должен штатно завершаться по сигналам: syscall.SIGTERM, syscall.SIGINT, syscall.SIGQUIT.
  - [x] Данные, которые находятся в процессе обработки на момент получения сигнала, должны быть успешно переданы агентом на сервер, а сервер должен успешно сохранить все несохранённые данные.
  - [x] **К концу спринта покрытие тестами не менее 70%.**
- **Iter24**. Работа с сетью.
  - [x] Добавьте в конфигурационный JSON-файл сервера поле `trusted_subnet` (тип `string`, переменная окружения `TRUSTED_SUBNET`, флаг `-t`), в которое можно передать строковое представление бесклассовой адресации (CIDR).
  - [x] Добавьте в запрос агента заголовок X-Real-IP, в котором должен содержаться IP-адрес хоста агента.
  - [x] При отправке метрик агентом серверу нужно проверять, что переданный в заголовке запроса X-Real-IP IP-адрес агента входит в доверенную подсеть, в противном случае возвращать статус ответа 403 Forbidden.
...
- **Iter25**. Обмен данными по протоколу gRPC.
  ...
  - [x] Добавление параметров конфигурации в сервер и агент
  - [x] Реализация gRPC сервера и клиента
    - [x] сервер
    - [x] агент
  - [x] Использование дополнительных возможностей gRPC:
    - [x] сжатие данных с помощью gRPC gzip
    - [x] подпись данных с помощью метаданных
    - [x] шифрование данных c помощью gRPC TLS
    - [x] потоковая передача данных
  - [x] **К концу спринта ваш код должен быть покрыт тестами на 80%**.

Общее покрытие тестами: **81.4%**.
...
```json
{
    ...
    "crypto_key": "/path/to/server_server.crt", // аналог переменной окружения CRYPTO_KEY или флага -k
