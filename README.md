...
- **Iter15**. Паттерны многопоточности.
    ...
    - [ ] **Агент**: Перепланируйте архитектуру агента таким образом, чтобы сбор метрик (опрос runtime) и их отправка осуществлялись в разных горутинах. 
    ...
    - [ ] **Агент**: Добавьте в сервер обработку запросов с использованием паттерна worker pool.
        - TotalMemory,
        -  FreeMemory,
